module.exports = async message => {
    ...
    if (!guild) {
        ...
        if (db && db[config.guilds.mhap] && db[config.guilds.mhap].welcomer && db[config.guilds.mhap].welcomer[author.id] && (content.toLowerCase().includes("yes") || content.toLowerCase().includes("no"))) {
            function processRole(role) {
                if (content.toLowerCase().includes("yes")) {
                    client.guilds.resolve(config.guilds.mhap).members.resolve(author.id).roles.add(config.roles[role]);
            ...
            const msg = await channel.messages.fetch(db[config.guilds.mhap].welcomer[author.id]);
            ...
            switch (embed.title) {
                case "Roles - Poll (Stage 1)":
                    ...
                    db[config.guilds.mhap].welcomer[author.id] = newMsg.id;
                    break;
                ...
                case "Roles - Changelog (Stage 3)":
                    ...
                    delete db[config.guilds.mhap].welcomer[author.id];
                    break;
            }
    ...
    if (channel.parent && channel.parent.id === config.categories.main.dmChannels) {
    ...
    if (message.webhookID && message.webhookID === config.getWebhookID()) {
        ...
        const guild = client.guilds.resolve(config.guilds.mhap);
        if (user) {
            ...
            async function handlePost(categoryTitle, channelName, channelTopic) {
                ...
                if (!postsCategory) {
                    ...
                    await postsCategory.setPosition(client.channels.resolve(config.categories.mhap.olympus).position - 1);
                }
                ...
                case "rawSupportTicket":
                    ...
                    if (restriction && restriction !== "EVERYONE!") {
                        let permissions = [
                            ...
                            {id: config.roles.mhap.muted, deny: ["SEND_MESSAGES"]},
                            {id: user.id, allow: ["VIEW_CHANNEL"]}
                        ];
                        if (restriction === "Staff only.") permissions.push({id: config.roles.mhap.staff, allow: ["VIEW_CHANNEL"]});
