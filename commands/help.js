    ...
    async execute(message, args) {
        ...
        if(command) {
            ...
            text = "**Command name:** " + command.name +
                ...
                (command.params ? "\n**Parameters:** " + command.params.all.map(a => "`" + a + "`").join(", ") + "\n**[required] (optional)**" : "");
        }
        // + "\nYou can edit the message to execute again.");
        else text = "Command not found!";
        message.channel.send(message.author.toString(), await fun.embed(message.client, "Command Help For: " + args[0], text)).then(async m => {
