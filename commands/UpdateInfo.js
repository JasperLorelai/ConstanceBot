module.exports = {
    ...
    async execute(Libs, message) {
        ...
        if (!data) throw new ConditionException(message, "Info Updater", "There is no guild data defined for this guild.");
        const infoChannelID = data.channels.info;
        if (!infoChannelID) throw new ConditionException(message, "Info Updater", "This guild has no info channel defined in its data.");
        const infoChannel = guild.channels.resolve(infoChannelID);
        if (!infoChannel) throw new ConditionException(message, "Info Updater", "The info channel defined for this guild does not exist in this guild.");
        ...
        if (!infoMsg) throw new ConditionException(message, "Info Updater", "The defined info message was not found in the info channel.");
        const infoData = data.info;
        if (!infoData) throw new ConditionException(message, "Info Updater", "This guild has no info message defined.");
        ...
        if (embed && embed.description === infoText.description && embed.fields === infoText.fields) {
            throw new ConditionException(message, "Info Updater", "Nothing was updated.");
        }
